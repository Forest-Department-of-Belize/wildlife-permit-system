...
export async function getOffenseByUuid(uuid: string) {
	const rows = await db
		.select({
			...
			eriConfiscationId: compoundOffenses.eriConfiscationId,
			offenseDate: compoundOffenses.offenseDate,
			illegalWildlife: compoundOffenses.illegalWildlife,
			...
			handTame: compoundOffenses.handTame,
			approxDateAcquired: compoundOffenses.approxDateAcquired,
			lessTwelveMonthsAcquired: compoundOffenses.lessTwelveMonthsAcquired,
