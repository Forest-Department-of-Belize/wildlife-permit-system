...
import { db } from "../db";
import { districts, parrotSpecies, ranges, users } from "../db/schema";
...
export async function createUser(data: CreateUserData) {
  ...
  const result = await db
    ...
    .values({
      ...
      email: data.email,
      roleId: data.roleId,
      rangeId: data.rangeId?.toString().trim() === "" ? null : data.rangeId,
      inviteToken,
...
export async function updateRange(
  ...
  const result = await db
    .update(ranges)
    .set({ name, districtId, updatedAt: sql`NOW()` })
...
export async function updateSpecies(
  ...
  const result = await db
    .update(parrotSpecies)
    .set(updates)
    .where(eq(parrotSpecies.uuid, uuid))
    .returning();
  return result[0];
}
